...
function openCaseModal(existingCase = null) {
    ...
    // Date formatting for datetime-local inputs (YYYY-MM-DDTHH:mm)
    ...
    let existingHearings = (existingCase?.hearings || (existingCase?.nextHearing ? [existingCase.nextHearing] : [])).map(h => formatToISO(h));
...
function viewDetailsModal(caseRecord) {
    ...
    const d = parseDateSafe(caseRecord.nextHearing);
    let nh = d ? d.toLocaleString('en-US', { month: 'long', day: 'numeric', year: 'numeric', hour: '2-digit', minute: '2-digit' }) : 'No scheduled hearing';
    ...
    const fullHtml = `> ACCESSING ENCRYPTED RECORD...\n` +
                 ...
                 `<span class="highlight-accused">[ ${complainantType.toUpperCase()} ] : ${String(caseRecord.complainant || 'N/A').replace(/ \| /g, ', ')}</span>` +
                 `<span class="highlight-accused">[ ${accusedType.toUpperCase()} ] : ${String(caseRecord.respondent || 'N/A').replace(/ \| /g, ', ')}</span>\n` +
